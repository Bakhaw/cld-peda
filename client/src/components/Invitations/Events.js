...
class Events extends Component {
  ...
  render() {
    ...
    const { allEvents, appLoading, availablesEvents, selectedEventOnEventsListId } = contextState;
        ...
        <List className="invitations-list-container">
          {availablesEvents.length > 0 &&
            allEvents.map((item, i) => {
              ...
              return (
                <ListItem button key={i} onClick={() => selectEvent(item)} selected={selectedEventOnEventsListId === item._id}>
                  <ListItemIcon>
                    <CalendarIcon />
                  </ListItemIcon>
        ...
        {!isAdmin && (
          <div className="chose-event">
            <Button
              ...
              onClick={e => handleAddNewDatesInCalendarSubmit(e, eventTitle, history)}
              size="large"
              type="submit"
              variant="extendedFab"
            >
