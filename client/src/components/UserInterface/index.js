  ...
  render() {
    ...
    return (
      ...
      <div className="user-container">
        {step === 1 && (
          <UserForm handleChangeInput={this.handleUserFormInputChange} handleFormSubmit={this.handleUserFormSubmit} lastname={lastname} name={name} />
        )}

        {step === 2 && (
          <div className="choose-date">
            <Typography variant="h1">Choisissez vos dates, {name.value}</Typography>
            <Invitations eventTitle={`${lastname.value} ${name.value}`} history={history} isAdmin={false} />
          </div>
        )}
      </div>
    );
